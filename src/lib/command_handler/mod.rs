...
pub fn run_macro(entry: &MacroType) {
    ...
    match entry {
        ...
        MacroType::Command(cmd) => {
            let _ = handle_command(&cmd, &return_value);
        }
        MacroType::MetaCommand(cmd) => {
            for command in &cmd.commands {
                return_value = handle_command(&command, &return_value);
            }
        }
    }
...
fn handle_command(
    ...
    match command {
        MacroCommand::LatestDir(cmd) => {
            return handle_command_newest_directory(&return_value, &cmd)
        }
        MacroCommand::LatestFile(cmd) => {
            return handle_command_newest_file(&return_value, &cmd)
        }
        MacroCommand::Cmd(cmd) => {
            return handle_command_cmd(&return_value, &cmd)
        }
    };
}
